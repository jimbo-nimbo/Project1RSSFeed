...
public class WebSiteRepositoryTest
{
    ...
    public void addWebSiteInjectionTest()
    {
        try (Connection conn = Core.getInstance().getDatabaseConnectionPool().getConnection())
        {
            ...
            conn.createStatement().execute(WebsiteTableQueries.CREATE_WEBSITE_TABLE_IF_NOT_EXISTS.toString());
            conn.createStatement().execute(RSSItemTableQueries.CREATE_RSSITEM_TABLE_IF_NOT_EXISTS.toString());
            Thread.sleep(5000);
            addWebSite();

            ResultSet resultSet = conn.createStatement().executeQuery(WebsiteTableQueries.SELECT_ALL_WEBSITES.toString());
